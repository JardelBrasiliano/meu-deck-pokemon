...
export function* listPokemon({ payload }) {
  try {
    ...
    const limit = 8;
    const { data } = yield call(apiListPokemon.get, `/pokemon-form/?offset=${offSet}&limit=${limit}`);
    const listInitial = data.results;
    // Pegando dados da api com "yield all" para usar o .map e percorrer todas as informações.
    const listPokeminFinal = yield all(listInitial.map((pokemon) => call(apiListPokemon.get, `/pokemon-form/${pokemon.name}`)));
